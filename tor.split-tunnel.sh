...
nft add chain ip nat POSTROUTING "{type nat hook postrouting priority srcnat -1; policy accept;}"
nft add rule ip nat POSTROUTING oifname ${OIFNAME} meta cgroup 1114129 counter masquerade
nft add table ip mangle
nft add chain ip mangle markit "{type route hook output priority mangle -1; policy accept;}"
nft add rule ip mangle markit meta cgroup 1114129 counter meta mark set 0xb
nft add chain inet filter input "{type filter hook input priority filter -1; policy accept;}"
nft add chain inet filter output "{type filter hook output priority filter -1; policy accept;}"
